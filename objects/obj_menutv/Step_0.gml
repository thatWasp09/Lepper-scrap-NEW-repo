    ...
    case states.transition:
        if (floor(image_index) == (image_number - 1))
        {
            if selected
            {
                ...
                var _started = (obj_mainmenu.shownoise ? global.gameN[trigger].started : (obj_mainmenu.showlep ? global.gameL[trigger].started : global.game[trigger].started))
                if (!_started)
                {
                    sprite_index = transspr
...
if (showlep != obj_mainmenu.showlep)
    instance_create(x, y, obj_genericpoofeffect)
showlep = obj_mainmenu.showlep
