...
switch state
{
    ...
    case states.normal:
        ...
        else
        {
            ...
            if ((noise_unlocked || lep_unlocked) && (!key_jump) && (!key_delete2) && (!key_quit2))
            {
                var p = 5
                ...
                if (_move_v != 0)
                {
                    ...
                    if (charselect == 3 && (!swap_unlocked))
                        charselect = goback
                    if ((charselect == 2 && (!lep_unlocked)) || (charselect == 1 && (!noise_unlocked)))
                        charselect += _move_v
                    if (charselect != c)
                    {
                    ...
                    if (_noise != shownoise || _lep != showlep)
                    {
                        with (obj_menutv)
                        {
                            if selected
            ...
            else if (key_delete2 && (global.game[currentselect].started || global.gameN[currentselect].started || global.gameL[currentselect].started))
            {
                deletebuffer = 0
    ...
    case states.bombdelete:
        ...
        if ((deleteselect == 1 && key_jump) || (deleteselect == 0 && deletebuffer >= 120))
        {
            if (deleteselect == 0)
            {
                var file = concat(get_save_folder(), "/saveData", (currentselect + 1), ((!shownoise) ? (showlep ? "LEP.ini" : ".ini") : "N.ini"))
                if file_exists(file)
                    file_delete(file)
                if showlep
                    global.gameL[currentselect] = game_empty()
                else if (!shownoise)
...
if (currentselect != -1)
{
    ...
    lep_game = menu_get_game(currentselect, 2)
    if (state != states.titlescreen && state != states.transition)
    {
        ...
        a = (floor((abs(lep_percvisual - lep_game.percentage)) / 10)) + 1
        lep_percvisual = Approach(lep_percvisual, lep_game.percentage, a)
        lep_game.percvisual = lep_percvisual
    }
}
var acc = 2
game_icon_index += 0.1
...
if shownoise
{
    ...
    lep_alpha = Approach(lep_alpha, 0, acc)
}
else if showlep
{
    noise_alpha = Approach(noise_alpha, 0, acc)
    pep_alpha = Approach(pep_alpha, 0, acc)
    lep_alpha = Approach(lep_alpha, 1, acc)
}
else
{
    noise_alpha = Approach(noise_alpha, 0, acc)
