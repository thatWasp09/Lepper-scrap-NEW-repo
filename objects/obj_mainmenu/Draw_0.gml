...
var games = [pep_game, noise_game, lep_game]
...
draw_set_alpha(extrauialpha)
if (noise_unlocked || lep_unlocked)
{
    var icon = spr_mainmenu_pepicon
    if (!noise_unlocked)
        icon = spr_mainmenu_pepleppericon
    if shownoise
    {
        ...
        if lep_unlocked
            icon = spr_mainmenu_noiseleppericon
        if showswap
        {
            icon = spr_mainmenu_swapicon
            if lep_unlocked
                icon = spr_mainmenu_swapleppericon
        }
    }
    if showlep
    {
        icon = spr_mainmenu_leppericon
        if (!noise_unlocked)
            icon = spr_mainmenu_lepperpepicon
        if swap_unlocked
            icon = spr_mainmenu_lepperswapicon
    }
...
if (currentselect != -1)
{
    ...
    if showlep
        _game = global.gameL[currentselect]
    if (!_game.started)
        dal = 0.5
}
