function scr_player_bombgrab() //gml_Script_scr_player_bombgrab
{
    ...
    if (sprite_index != spr_uppercutfinishingblow && sprite_index != spr_player_throw && sprite_index != spr_playerL_bombthrow)
        hsp = move * movespeed
    else
        hsp = 0
    if (hsp != 0)
    ...
    if (!instance_exists(bombgrabID))
        state = states.normal
    if (sprite_index != spr_haulingstart && sprite_index != spr_uppercutfinishingblow && sprite_index != spr_player_throw && sprite_index != spr_playerL_bombthrow)
    {
        if (grounded && vsp > 0)
        {
            if (hsp != 0)
                ...
                sprite_index = spr_playerL_bombidle
        }
        ...
        if (sprite_index == spr_haulingjump && floor(image_index) == (image_number - 1))
            sprite_index = spr_haulingfall
        if (input_buffer_slap > 0 && ispeppino && (!islepper))
        {
            input_buffer_slap = 0
            image_index = 0
            if key_up
            ...
            {
                sprite_index = spr_playerL_bombthrow
                with (bombgrabID)
                {
                    state = states.normal
                    movespeed = 11
        ...
        if (input_buffer_slap > 0 && islepper)
        {
            input_buffer_slap = 0
            sprite_index = spr_playerL_bombidle
            image_index = 0
            ...
            movespeed = 0
            vsp = 0
            lepperbombthrow = true
            lepperaim = 0
        }
        else if (key_down && lepperbombthrow == false)
        {
            if grounded
            {
                state = states.normal
    ...
    if (islepper && lepperbombthrow == true)
    {
        if key_up
            lepperaim += 5
        else if key_down
