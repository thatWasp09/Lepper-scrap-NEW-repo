function scr_player_tumble()
{
    ...
    if ((!grounded) && (sprite_index == spr_crouchslip || sprite_index == spr_playerL_machrollbegin || sprite_index == spr_machroll || sprite_index == spr_mach2jump || sprite_index == spr_backslide || sprite_index == spr_backslideland))
    {
        ...
        else
        {
            ...
            if islepper
            {
                ...
                sprite_index = spr_playerL_divestart
                image_index = 0
                vsp = -4
            }
            else
    ...
    if (grounded && (sprite_index == spr_dive || sprite_index == spr_playerL_divestart) && ((!(place_meeting(x, (y + vsp), obj_destructibles))) || (!islepper)))
    {
        if islepper
        {
    ...
    if ((sprite_index == spr_dive || sprite_index == spr_playerL_divestart) && islepper)
    {
        if (sprite_index == spr_dive)
        {
            ...
            vsp -= 0.2
        movespeed = Approach(movespeed, (move * 6 * xscale), 0.7)
    }
    if ((sprite_index == spr_dive || sprite_index == spr_playerL_divestart) && key_jump)
    {
        if ispeppino
        {
            input_buffer_jump = 0
            ...
            if islepper
            {
                sprite_index = spr_playerL_divebomb
                if (vsp < 10)
                    vsp = 10
                freefallsmash = 10
            }
    ...
    if (movespeed <= 2 && sprite_index != spr_tumble && sprite_index != spr_breakdance && ((sprite_index != spr_dive && sprite_index != spr_playerL_divestart) || (!islepper)))
        state = states.normal
    ...
    if (sprite_index == spr_dive && vsp < 10 && (!islepper))
        vsp = 10
}